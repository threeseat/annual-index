...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="bookauthor-full-name">
		<xsl:for-each select="item/contrib[@contrib-type = 'bookauthor' and not(@role)]">
			<xsl:value-of select="concat(string-name/given-names, string-name/surname, string-name/suffix)" separator=" "/>
		</xsl:for-each>		
	</xsl:variable>
	
	
	<xsl:variable name="bookeditor-full-name">
		<xsl:for-each select="item/contrib[@contrib-type = 'bookauthor' and @role = 'bookeditor']">
			<xsl:value-of select="concat(string-name/given-names, string-name/surname, string-name/suffix)" separator=" "/>
		</xsl:for-each>		
	</xsl:variable>
	
	<xsl:variable name="booktranslator-full-name">
		<xsl:for-each select="item/contrib[@contrib-type = 'bookauthor' and @role = 'booktranslator']">
			<xsl:value-of select="concat(string-name/given-names, string-name/surname, string-name/suffix)" separator=" "/>
		</xsl:for-each>		
	</xsl:variable>
	
	<xsl:variable name="bookreviewer-full-name">
		<xsl:for-each select="item/contrib[@contrib-type = 'reviewer']/string-name">
			<xsl:value-of select="concat(string-name/given-names, string-name/surname, string-name/suffix)" separator=" "/>
		</xsl:for-each>		
	</xsl:variable>
...
color: green;
			...
			<body>
				<h1><xsl:call-template name="year"/> Annual Index Books Reviewed by Subject</h1>
	...
	<xsl:template name="main">
		<xsl:for-each select="/authority/book_categories/book_category/title">
			<xsl:sort order="ascending" data-type="text"/>
			<h3>
				<xsl:value-of select="." />
			</h3>
			<xsl:for-each-group
				select="/authority/item[index-cat = current()/../code]"
				group-by="title">
				<xsl:sort select="replace(title, '^(An |A |The )', '')"/>
				<li>
					<em>
						<xsl:value-of select="title" />
					</em>
					<xsl:text>, </xsl:text>
					<xsl:call-template name="bookauthors"/>
					<xsl:call-template name="bookeditors"/>
					<xsl:call-template name="booktranslators"/>
					<xsl:call-template name="bookreviewer"/>
					<xsl:call-template name="month-page"/>
				</li>
			</xsl:for-each-group>
		</xsl:for-each>
	</xsl:template>

	<xsl:template name="bookauthors" match="/authority/item">
		<xsl:for-each
			select="contrib[@contrib-type = 'bookauthor' and not(@role)]">
			<xsl:choose>
				<xsl:when
					test="preceding-sibling::contrib[@contrib-type = 'bookauthor' and not(@role)]">
					<xsl:text>, </xsl:text>
				</xsl:when>
			</xsl:choose>
