...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="author" xml:space="preserve"><xsl:choose><xsl:when test="/article/front/article-meta/contrib-group/contrib[@contrib-type = 'reviewer']"><contrib contrib-type="reviewer"><xsl:copy-of select="/article/front/article-meta/contrib-group/contrib/name" /></contrib><xsl:call-template name="bookauthors" /></xsl:when><xsl:otherwise><xsl:for-each select="//article/front/article-meta/contrib-group/contrib"><contrib><xsl:attribute name="contrib-type"><xsl:value-of select="@contrib-type" /></xsl:attribute><string-name><xsl:attribute name="name-style"><xsl:value-of select="name/@name-style" /></xsl:attribute><xsl:copy-of select="name/*" /></string-name></contrib></xsl:for-each></xsl:otherwise></xsl:choose></xsl:template>
	...
	<xsl:template name="bookauthors" match="/article/front/article-meta/product">
		<xsl:for-each select="/article/front/article-meta/product/string-name">
			<contrib contrib-type="bookauthor">
				<xsl:copy-of select="." />
			</contrib>
		</xsl:for-each>
	</xsl:template>
